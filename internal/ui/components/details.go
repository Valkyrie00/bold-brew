...
func NewDetails(theme *theme.Theme, brewService BrewServiceInterface) *Details {
	details := &Details{
...
func (d *Details) SetFormula(info *models.Formula) {
	if info == nil {
		d.view.SetText("")
		return
	}
...
func (d *Details) getPackageInstallationDetails(info *models.Formula) string {
	...
	if d.brewService != nil {
		if path, err := d.brewService.GetPrefixPath(info.Name); err == nil {
			packagePrefix = path
		}
	}
